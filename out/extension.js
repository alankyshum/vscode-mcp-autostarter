...
async function restartAllFailedServers() {
    ...
    const failedServers = servers.filter(config => serverManager.getServerStatus(config.id) === 'error');
...
async function addNewServer() {
    try {
        ...
        if (servers[serverId]) {
            const overwrite = await vscode.window.showWarningMessage(`A server with ID "${serverId}" already exists. Do you want to overwrite it?`, { modal: true }, 'Overwrite');
...
function showPerformanceMetrics() {
    ...
    const panel = vscode.window.createWebviewPanel('mcpPerformanceMetrics', 'MCP Auto-Starter Performance Metrics', vscode.ViewColumn.One, {});
    panel.webview.html = `
        <!DOCTYPE html>
        <html>
        <head>
